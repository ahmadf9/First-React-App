...
export const populateCars = (cars) => {
  return cars.map((car) => {
    ...
    const availableAt = randomDate(new Date(2022, 5, 25), new Date(2022, 6, 5)).toISOString();

    return {
      ...car,
      availableAt,
      driver: ["withDriver", "noDriver"][Math.floor(Math.random() * ["withDriver", "noDriver"].length)],
    };
  });
};
